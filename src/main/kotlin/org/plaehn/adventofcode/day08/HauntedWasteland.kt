...
class HauntedWasteland(
    ...
    fun countSteps(): Int {
        directions.asSequence().cycle().fold("AAA" to 0) { (position, stepCount), direction ->
            val newPosition = position2NextPositions[position]!![direction]
            val newStepCount = stepCount + 1

            if (newPosition == "ZZZ") return newStepCount
    ...
    data class Node(
        ...
        companion object {
            ...
            private val inputMatchingRegex = "([A-Z]{3}) = \\(([A-Z]{3}),\\s+([A-Z]{3})\\)".toRegex()
